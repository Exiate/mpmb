...
AddSubClass("artificer", "Demolitionist", {
	...
	features : {
		...
		"subclassfeature9" : {
			...
			action : ["bonus action", ""],
			calcChanges : {
				atkAdd : ["if ((classes.known.artificer && classes.known.artificer.level > 8 && (/basic/i).test(WeaponName)) {fields.Range = (fields.Range ? '' : '') + '60ft(10ft)' } || (classes.known.artificer && classes.known.artificer.level > 8 && (/advanced/i).test(WeaponName)) {fields.Range = (fields.Range ? '' : '') + '60ft(15ft)');"]
			}
		},
		//"subclassfeature15" : {
		//	name : "Apex",
