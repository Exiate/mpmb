...
AddSubClass("artificer", "Demolitionist", {
	...
	features : {
		...
		"subclassfeature5" : {
			...
			calcChanges : {
				atkAdd : ["if (classes.known.artificer && classes.known.artificer.level > 4 && ((/basic explosive/i).test(fields.Description) || (/basic explosive/i).test(WeaponName))) {fields.Description += (fields.Description ? '; ' : '') + levels.map(HomebrewPowderDie) + ' damage'};"]
			}
		},
		//"subclassfeature9" : {
