...
AddSubClass("artificer", "Demolitionist", {
	...
	features : {
		...
		"subclassfeature5" : {
			...
			calcChanges : {
				atkAdd : ["if (classes.known.artificer && classes.known.artificer.level > 4 && ((/basic explosive/i).test(fields.Description) || (/basic explosive/i).test(WeaponName))) {fields.Description += (fields.Description ? '; ' : '') + levels.map(n < 11 ? 1 : n < 17 ? 2 : 3) + 'd4 damage'};"]
			}
		},
		//"subclassfeature9" : {
