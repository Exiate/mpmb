...
AddSubClass("monk", "Way of the Dark Star", {
	...
	features : {
		"subclassfeature1" : {
			...
			calcChanges : {
				atkAdd : ["if (classes.known.monk && classes.known.monk.level > 2 && (/unarmed/i).test(WeaponName)) {fields.Range = ''}"]
				atkAdd : ["if (classes.known.monk && classes.known.monk.level > 2 && (/unarmed/i).test(WeaponName)) {fields.DamageType = 'Force'}"]
			},
		},
		"subclassfeature1.1" : {
			name : "Devour Energy",
